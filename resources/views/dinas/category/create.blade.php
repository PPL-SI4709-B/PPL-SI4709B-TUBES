...
<div class="flex flex-col gap-6" style="max-width: 640px;">
    ...
    <div class="form-card">
        <h2 class="font-bold mb-6" style="font-size: var(--text-lg); color: var(--color-text-dark);">Tambah Kategori Baru</h2>

        <form action="{{ route('dinas.category.store') }}" method="POST" id="form-create-category" dusk="category-form">
            @csrf

            <div class="mb-4">
                <label for="name" class="input-label mb-2">NAMA KATEGORI <span style="color: var(--color-status-reject-text);">*</span></label>
                <input type="text" name="name" id="name" class="input-field" value="{{ old('name') }}" placeholder="Contoh: Kuliner, Fashion, Kerajinan" required dusk="category-name">
                @error('name')
            ...
            <div class="mb-6">
                ...
                <textarea name="description" id="description" class="input-field" rows="4" placeholder="Deskripsi singkat kategori usaha (opsional)" style="resize: vertical;" dusk="category-description">{{ old('description') }}</textarea>
                @error('description')
            ...
            <div class="flex gap-3">
                <button type="submit" class="btn btn-brand" id="btn-simpan" dusk="category-submit">
