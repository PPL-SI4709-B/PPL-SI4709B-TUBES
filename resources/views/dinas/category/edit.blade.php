...
<div class="flex flex-col gap-6" style="max-width: 640px;">
    ...
    <div class="card" style="padding: var(--space-6);">
        ...
        <form action="{{ route('dinas.category.update', $category) }}" method="POST" id="form-edit-category">
            @csrf
            @method('PUT')

            <div class="mb-4">
                <label for="name" class="input-label mb-2">NAMA KATEGORI <span style="color: var(--color-status-reject-text);">*</span></label>
                <input type="text" name="name" id="name" class="input-field" value="{{ old('name', $category->name) }}" placeholder="Contoh: Kuliner, Fashion, Kerajinan" required>
                @error('name')
            ...
            <div class="mb-6">
                ...
                <textarea name="description" id="description" class="input-field" rows="4" placeholder="Deskripsi singkat kategori usaha (opsional)" style="resize: vertical;">{{ old('description', $category->description) }}</textarea>
                @error('description')
                    <p style="color: var(--color-status-reject-text); font-size: var(--text-xs); margin-top: var(--space-1); font-weight: 500;">{{ $message }}</p>
                @enderror
            </div>
