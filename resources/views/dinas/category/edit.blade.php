...
@section('title', 'Edit Kategori Usaha - Portal UMKM')
...
<header class="main-header">
    <div class="page-title">Edit Kategori Usaha</div>
...
<div class="flex flex-col gap-6" style="max-width: 640px;">
    ...
    <div class="form-card">
        <h2 class="font-bold mb-6" style="font-size: var(--text-lg); color: var(--color-text-dark);">Edit Kategori</h2>

        <form action="{{ route('dinas.category.update', $category) }}" method="POST" id="form-edit-category" dusk="category-form">
            @csrf
            @method('PUT')

            <div class="mb-4">
                <label for="name" class="input-label mb-2">NAMA KATEGORI <span style="color: var(--color-status-reject-text);">*</span></label>
                <input type="text" name="name" id="name" class="input-field" value="{{ old('name', $category->name) }}" placeholder="Contoh: Kuliner, Fashion, Kerajinan" required dusk="category-name">
                @error('name')
            ...
            <div class="mb-6">
                ...
                <textarea name="description" id="description" class="input-field" rows="4" placeholder="Deskripsi singkat kategori usaha (opsional)" style="resize: vertical;" dusk="category-description">{{ old('description', $category->description) }}</textarea>
                @error('description')
            ...
            <div class="flex gap-3">
                <button type="submit" class="btn btn-brand" id="btn-update" dusk="category-submit">
