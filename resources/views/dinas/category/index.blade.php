            ...
            <table style="width: 100%; border-collapse: collapse;">
                <thead>
                    <tr style="background-color: var(--color-input-bg); border-bottom: 1px solid var(--color-border);">
                        ...
                        <th style="padding: var(--space-3) var(--space-4); text-align: left; font-size: 11px; font-weight: 700; color: var(--color-text-muted); text-transform: uppercase; letter-spacing: 0.05em;">Nama Kategori</th>
                        ...
                        <tr>
                            <td colspan="5" style="padding: var(--space-8); text-align: center; color: var(--color-text-muted); font-size: var(--text-sm);">
                                <div class="flex flex-col items-center gap-3">
                                    ...
                                    <span>Belum ada kategori usaha.</span>
                                    <a href="{{ route('dinas.category.create') }}" class="btn btn-brand" style="font-size: var(--text-sm);">Tambah Kategori Pertama</a>
                                </div>
                            </td>
                        </tr>
                    @endforelse
        ...
        @if($categories->hasPages())
            <div style="padding: var(--space-3) var(--space-4); border-top: 1px solid var(--color-border); display: flex; justify-content: space-between; align-items: center;">
                ...
                <div class="flex gap-1">
                    ...
                    @else
                        <a href="{{ $categories->previousPageUrl() }}" style="padding: 6px 12px; font-size: var(--text-sm); color: var(--color-primary); border: 1px solid var(--color-border); border-radius: var(--radius-sm);">&laquo;</a>
                    @endif
                    ...
                    @if($categories->hasMorePages())
                        <a href="{{ $categories->nextPageUrl() }}" style="padding: 6px 12px; font-size: var(--text-sm); color: var(--color-primary); border: 1px solid var(--color-border); border-radius: var(--radius-sm);">&raquo;</a>
                    @else
