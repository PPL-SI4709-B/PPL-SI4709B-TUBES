...
<div class="flex flex-col gap-6" dusk="region-index">
    ...
    <div class="flex justify-between items-center">
        ...
        <a href="{{ route('dinas.region.create') }}" class="btn btn-brand" id="btn-tambah-wilayah" dusk="region-create-link">
            ...
            <table style="width: 100%; border-collapse: collapse;">
                ...
                <tbody>
                    @forelse($regions as $region)
                        <tr dusk="region-row-{{ $region->id }}" style="border-bottom: 1px solid var(--color-border);" onmouseover="this.style.backgroundColor='var(--color-input-bg)'" onmouseout="this.style.backgroundColor='transparent'">
                            ...
                            <td style="padding: var(--space-3) var(--space-4); text-align: right;">
                                <div class="flex justify-end gap-2">
                                    <a href="{{ route('dinas.region.edit', $region) }}" title="Edit" style="color: var(--color-primary); padding: 4px;" id="btn-edit-{{ $region->id }}" dusk="region-edit-{{ $region->id }}">
                                    ...
                                    <form action="{{ route('dinas.region.destroy', $region) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus wilayah ini?');" style="display: inline;">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" title="Hapus" style="color: var(--color-status-reject-text); padding: 4px; cursor: pointer; background: none; border: none;" id="btn-delete-{{ $region->id }}" dusk="region-delete-{{ $region->id }}">
