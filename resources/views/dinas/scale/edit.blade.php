...
<div class="flex flex-col gap-6" style="max-width: 640px;">
    ...
    <div class="form-card">
        ...
        <form action="{{ route('dinas.scale.update', $scale) }}" method="POST" id="form-edit-scale" dusk="scale-form">
            @csrf
            @method('PUT')

            <div class="mb-4">
                <label for="name" class="input-label mb-2">NAMA SKALA USAHA <span style="color: var(--color-status-reject-text);">*</span></label>
                <input type="text" name="name" id="name" class="input-field" value="{{ old('name', $scale->name) }}" placeholder="Contoh: Mikro, Kecil, Menengah" required dusk="scale-name">
                @error('name')
            ...
            <div class="mb-6">
                ...
                <textarea name="description" id="description" class="input-field" rows="4" placeholder="Deskripsi singkat skala usaha (opsional)" style="resize: vertical;" dusk="scale-description">{{ old('description', $scale->description) }}</textarea>
                @error('description')
            ...
            <div class="flex gap-3">
                <button type="submit" class="btn btn-brand" id="btn-update" dusk="scale-submit">
